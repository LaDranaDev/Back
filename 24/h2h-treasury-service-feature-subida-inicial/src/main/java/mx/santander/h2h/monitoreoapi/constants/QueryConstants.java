...
public final class QueryConstants {
    ...
    SELECT new mx.santander.h2h.monitoreoapi.model.response.ResumenProductoEstatusDTO(
...
""";
    ...
    SELECT new mx.santander.h2h.monitoreoapi.model.response.TotalGlobalOperacionesDTO(
        COUNT(p),
...
""";
    ...
    SELECT new mx.santander.h2h.monitoreoapi.model.response.ResumenProductoDivisaEstatusDTO(
...
""";
    ...
    SELECT new mx.santander.h2h.monitoreoapi.model.response.ResumenEstatusGlobalDTO(
        p.statusOper.descripcion,
        COUNT(p)
    )
    FROM OperPagoEntity p
...
""";
    ...
    SELECT new mx.santander.h2h.monitoreoapi.model.response.MontoTotalDivisaDTO(
        p.identificadores.divisa,
        COALESCE(SUM(p.montos.importe),0)
    )
    FROM OperPagoEntity p
...
SELECT new mx.santander.h2h.monitoreoapi.model.response.PagoDetalleDTO(
...
SELECT new mx.santander.h2h.monitoreoapi.model.response.ImporteTotalDivisaDTO(
    p.identificadores.divisa,
    COALESCE(SUM(p.montos.importe),0)
)
FROM OperPagoEntity p
...
""";
    ...
    public static final String DIVISAS_OPCION_CATALOGO = """
        SELECT new mx.santander.h2h.monitoreoapi.model.response.OpcionCatalogo(
            d.idDiv,
            d.txtDiv
        )
        FROM DivisaEntity d
